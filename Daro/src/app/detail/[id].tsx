...
import { I18nManager, Share, StyleSheet, Text, TouchableOpacity, View, useWindowDimensions } from 'react-native';
import Animated, {
  interpolate,
        ...
        options={{
          ...
          headerLeft: () => (
            <TouchableOpacity style={styles.barIcon} onPress={() => navigation.goBack()}>
              <Ionicons name={I18nManager.isRTL ? 'arrow-forward' : 'arrow-back'} size={18} />
            </TouchableOpacity>
          ),
          headerRight: () => (
        ...
        {isPending ? (
          ...
          <>
            <View style={styles.intro}>
              <Text style={styles.roomType}>{t('detail.roomType')}</Text>
              <Text style={[styles.name, { fontFamily: getFontFamily('bold', i18n.language) }]}>
                {data.title}
              </Text>
